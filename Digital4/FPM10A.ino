...
bool FPM10A::leSysParam (SYSPARAM *param) {
  ...
  if (recebeResp() && (resp.c.ack.codigo == RESP_OK)) {
    memcpy (param, &resp.c.ack.d.sysparam, sizeof(SYSPARAM));
    return true;
  } else {
    return false;
  }
...
int FPM10A::leTemplateCount() {
  ...
  enviaCmd ((byte *) &cmd, sizeof(cmd));
  if (recebeResp() && (resp.c.ack.codigo == RESP_OK)) {
    return (resp.c.ack.d.templcount[0] << 8)+ resp.c.ack.d.templcount[1];
  } else {
    return -1;
  }
}
...
bool FPM10A::capturaDigital () {
  ...
  cmd.cmd = CMD_GENIMG;
  enviaCmd ((byte *) &cmd, sizeof(cmd));
  return recebeResp() && (resp.c.ack.codigo == RESP_OK);
}
 ...
 * 'posicao' (0 a 149) da memória do módulo
 */
bool FPM10A::recuperaDigital (byte numBuf, int posicao) {
  CMDLOAD cmd;
  cmd.cmd = CMD_LOAD;
  cmd.bufno = numBuf;
  cmd.posicao[0] = posicao >> 8;
 ...
 * Se achar, retorna a posição (0 a 149)
 * Se não achar (ou der erro) retorna -1
 */
int FPM10A::procuraDigital (byte numBuf, int posicaoInicial, int qtdePosicoes) {
  CMDSEARCH cmd;
  cmd.cmd = CMD_SEARCH;
  cmd.bufno = numBuf;
  cmd.posicaoInicial[0] = posicaoInicial >> 8;
  cmd.posicaoInicial[1] = posicaoInicial & 0xFF;
  cmd.qtdePosicoes[0] = qtdePosicoes >> 8;
  cmd.qtdePosicoes[1] = qtdePosicoes & 0xFF;
  enviaCmd ((byte *) &cmd, sizeof(cmd));
  if (recebeResp() && (resp.c.ack.codigo == RESP_OK)) {
    return (resp.c.ack.d.busca.posicao[0] << 8) + resp.c.ack.d.busca.posicao[1];
  } else {
    return -1;
  }
}
...
/**
 * Le o conteudo do buffer de imagem
 * A rotina rotTx é chamada a cada bloco da imagem
 */
bool FPM10A::leImagem (ROT_TX rotTx) {
  CMDSEMPARAM cmd;
  cmd.cmd = CMD_UPIMG;
  enviaCmd ((byte *) &cmd, sizeof(cmd));
  if (!recebeResp() || (resp.c.ack.codigo != RESP_OK)) {
  ...
  while (true) {
    ...
    int tam = (resp.header.tamHi << 8) + resp.header.tamLo - 2;
    rotTx(resp.c.dados, tam);
    if (resp.header.tipo == TIPO_ENDDAT) {
      return true; 
    }
  }
}

// Le a feature ou modelo no buffer numBuf
bool FPM10A::leFeature (byte numBuf, ROT_TX rotTx) {
  CMDUPCHAR cmd;
  cmd.cmd = CMD_UPCHAR;
  cmd.bufno = numBuf;
  enviaCmd ((byte *) &cmd, sizeof(cmd));
...
// Grava a feature ou modelo no buffer numBuf
bool FPM10A::gravaFeature (byte numBuf, ROT_RX rotRx) {
  CMDUPCHAR cmd;
  cmd.cmd = CMD_DNCHAR;
  cmd.bufno = numBuf;
  enviaCmd ((byte *) &cmd, sizeof(cmd));
  ...
  bool fim = false;
  do {
    int tam = (*rotRx)(bufDados);
    if (tam < 0) {
      tam = -tam;
      fim = true;
    }
    enviaCmd(bufDados, tam, fim? TIPO_ENDDAT : TIPO_DAT);
  } while (!fim);
}

// Envia um comando para o sensor
void FPM10A::enviaCmd (byte *cmd, int tam, byte tipo) {
